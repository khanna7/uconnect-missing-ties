...
myfit <- ergm(mynet~edges,#+nodecov('greg'),
              target.stats=c(mdeg*n/2)) #,1.1*mdeg*n/2))
                                      # The 1.1 means that those with higher 'gregariousness' will form more ties.
...
mynet.mat[(nresp+1):n,(nresp+1):n] <- NA
mynet <- as.network.matrix(mynet.mat, matrix.type="adjacency", directed=FALSE)
summary(mynet~edges)
